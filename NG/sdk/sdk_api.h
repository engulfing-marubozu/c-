...
namespace hgng_sdk
{
    ...
    // Global variables to store handlers
    inline IOrderEventsHandler *g_order_events_handler = nullptr;
    inline IMarketDataHandler *g_market_data_handler = nullptr;
    ...
    {
        // Store the handler
        g_order_events_handler = handler;
    ...
    // Function to simulate an order event
    inline void TriggerOrderEvent(const OrderEvent &event)
    {
        if (g_order_events_handler)
        {
            g_order_events_handler->OnOrderEvent(event);
        }
    }

    // Function to simulate a price update
    inline void TriggerPriceUpdate(const PriceUpdate &price)
    {
        if (g_market_data_handler)
        {
            g_market_data_handler->OnPrice(price);
        }
    }
